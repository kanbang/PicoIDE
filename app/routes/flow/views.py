...
async def list_flows(business: Annotated[str, Depends(get_business)]):
    """
    获取所有 flows
    """
    try:
...
async def create_new_flow(request: CreateFlowRequest, business: Annotated[str, Depends(get_business)]):
    ...
    """
    try:
        flow = await create_flow(USER_ID, business, request.name, request.flow)
...
async def get_flow_by_id(flow_id: UUID, business: Annotated[str, Depends(get_business)]):
    """
    获取单个 flow
    """
    try:
        flow = await get_flow(USER_ID, business, flow_id)
        if not flow:
...
async def update_flow_by_id(flow_id: UUID, request: UpdateFlowRequest, business: Annotated[str, Depends(get_business)]):
    """
    更新 flow
    """
    try:
        success = await update_flow(USER_ID, business, flow_id, request.name, request.flow)
        if not success:
            raise HTTPException(404, "Flow not found")
        flow = await get_flow(USER_ID, business, flow_id)
        return to_flow_item(flow)
...
async def delete_flow_by_id(flow_id: UUID, business: Annotated[str, Depends(get_business)]):
    """
    删除 flow
    """
    try:
        success = await delete_flow(USER_ID, business, flow_id)
        if not success:
            raise HTTPException(404, "Flow not found")
        return {"ok": True}
    except HTTPException:
...
async def duplicate_flow_by_id(flow_id: UUID, request: DuplicateFlowRequest, business: Annotated[str, Depends(get_business)]):
    """
    复制 flow
    """
    try:
        flow = await duplicate_flow(USER_ID, business, flow_id, request.name)
        if not flow:
            raise HTTPException(404, "Flow not found")
        return to_flow_item(flow)
    except HTTPException:
