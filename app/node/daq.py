...
class SignalData:
    ...
    special_data: Optional[Any] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # 如果有特殊数据（如OrderMap），直接返回特殊格式

        if self.special_data is not None:

            return {"type": self.type, "data": self.special_data}

        # 标准XY格式

        return {
            "type": self.type,
            "data": {"x": self.x, "y": self.y, "meta": self.meta.to_dict()},
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SignalData":
        ...
        data_type = data.get("type", "time")

        inner = data.get("data", {})

        # 检查是否为OrderMap格式

        if data_type == "order_map" and isinstance(inner, list):
        ...
        return cls(
            x=inner.get("x", []), y=inner.get("y", []), meta=meta, type=data_type
        )
    ...
    def validate(self) -> Tuple[bool, str]:
        ...
        if self.type == "order_map" and self.special_data is not None:

            if not isinstance(self.special_data, list):

                return False, "OrderMap数据必须是列表"

            if len(self.special_data) == 0:

                return False, "OrderMap数据为空"

            for i, item in enumerate(self.special_data):

                if not isinstance(item, dict):

                    return False, f"OrderMap第{i}项不是字典"

                if "rpm" not in item or "order" not in item or "mag" not in item:

                    return False, f"OrderMap第{i}项缺少必要字段"
...
class AdlinkBridge:
    ...
    def _generate_simulated_data(self, channel: int) -> SignalData:
        ...
        try:
            ...
            fs = (
                self.fSamplerate
                if self.fSamplerate > 0
                else DAQConfig.DEFAULT_SAMPLERATE
            )
            ...
            return SignalData(x=t.tolist(), y=y.tolist(), meta=meta, type="time")
    ...
    def _generate_pulse_signal(
        self, t: np.ndarray, instantaneous_rpm: np.ndarray, fs: float
    ) -> np.ndarray:
        """生成脉冲信号"""
        pulse_times = []
        current_angle = 0.0
...
class VibrationGenerator(BaseBlock):
    ...
    def _initialize_once(self):
        ...
        self._base_meta = SignalMetadata(
            ...
            domain=DomainType.TIME.value,
            data_type=DataType.VIBRATION.value,
            description="Real-time generated vibration signal",
        )
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            t_batch = np.arange(t_start, t_start + self._update_interval, self._dt)[:self._batch_points]
            n_batch = len(t_batch)
            if n_batch == 0:
                return
...
class TurbineSimulator(BaseBlock):
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        try:
            ...
            w_torsion_mode1[mask] = (
                (w_nom * 0.005)
                * decay
                * np.sin(2 * np.pi * f_mode1 * (t[mask] - t_impact))
            )

            # B. 电气二倍频纹波 (2 * 50Hz = 100Hz)
            w_torsion_elec = (w_nom * 0.001 * elec_ratio) * np.sin(
                2 * np.pi * (2 * grid_freq) * t
            )
            ...
            self.set_interface(
                "O-Pulse-XY",
                SignalData(
                    x=t.tolist(), y=pulse.tolist(), meta=pulse_meta, type="pulse"
                ).to_dict(),
            )
            ...
            self._logger.debug(
                f"Turbine simulation done. PPR={ppr}, AvgPoints={len(t_rpm_avg)}"
            )
...
class CSVReader(BaseBlock):
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            self.set_interface(
                "O-List-XY",
                SignalData(x=x_data, y=y_data, meta=meta, type="time").to_dict(),
            )
        ...
        try:
            ...
            if len(x_data) != len(y_data):
                raise DataValidationError(
                    f"X和Y长度不匹配: {len(x_data)} != {len(y_data)}"
                )
...
class TimeWindow(BaseBlock):
    ...
    def __init__(self):
        ...
        self.add_number_option(
            "窗口长度 (s)", default=1.0, min_val=0.001, max_val=1000.0
        )
        self.add_number_option("重叠率 (%)", default=0.0, min_val=0.0, max_val=90.0)
        self.add_select_option(
            "窗口函数",
        ...
        self.add_select_option(
            "输出模式", items=["首个窗口", "全部窗口"], default="首个窗口"
        )
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            if mode == "首个窗口":
                ...
                segments_meta = (
                    SignalMetadata(**meta) if isinstance(meta, dict) else meta
                )
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            self.set_interface(
                "O-List-XY",
                SignalData(
                    x=x.tolist(), y=y_filtered.tolist(), meta=new_meta, type="filtered"
                ).to_dict(),
            )
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            self.set_interface(
                "O-List-XY",
                SignalData(
                    x=x.tolist(), y=envelope.tolist(), meta=new_meta, type="envelope"
                ).to_dict(),
            )
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            self.set_interface(
                "O-List-XY",
                SignalData(
                    x=freqs.tolist(), y=mag.tolist(), meta=new_meta, type="spectrum"
                ).to_dict(),
            )
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            self.set_interface(
                "O-Spectrum",
                SignalData(
                    x=X.tolist(), y=y_avg.tolist(), meta=new_meta, type="spectrum"
                ).to_dict(),
            )
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            self.set_interface(
                "O-RPM-XY",
                SignalData(
                    x=t_mid.tolist(), y=rpm.tolist(), meta=meta, type="rpm"
                ).to_dict(),
            )
...
class AngularResampler(BaseBlock):
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            # 精确提取脉冲边沿时刻（亚采样插值）
            rising_idx = np.where(
                (v_pulse[:-1] < threshold) & (v_pulse[1:] >= threshold)
            )[0]
            ...
            angle_to_vib_func = interp1d(
                vib_angles, v_vib, kind="linear", bounds_error=False, fill_value=0
            )
            ...
            self._logger.debug(
                f"角域重采样完成: {total_revs:.2f} 转, {num_target_points} 点"
            )
...
class RPMTrackedFFT(BaseBlock):
    ...
    async def on_compute(self, execution_id: Optional[str] = None):
        ...
        try:
            ...
            if not np.any(mask):
                raise DataValidationError(
                    f"转速范围内无数据: {rpm_center} ± {rpm_bw} RPM"
                )
            ...
            self.set_interface(
                "O-Order-Spectrum",
                SignalData(
                    x=orders.tolist(), y=mag.tolist(), meta=meta, type="order_spectrum"
                ).to_dict(),
            )
